...
function hijriToGregorian(hY: number, hM: number, hD: number) {
  ...
  let i: number;
  let k: number;
  const gregDate = new Array(3);
  const jd = intPart((11 * hY + 3) / 30) + 354 * hY + 30 * hM - intPart((hM - 1) / 2) + hD + 1948440 - 385;
...
/** @hidden */
export const hijriCalendar: MbscCalendarSystem = {
  getYear(date) {
